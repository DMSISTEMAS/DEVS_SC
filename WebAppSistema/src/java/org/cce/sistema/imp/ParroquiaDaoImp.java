...
public class ParroquiaDaoImp implements ParroquiaDao {
	...
	public List<String> listarParroquia(String nombre) {
		...
		Session session = HibernateUtil.getSf().openSession();
		Transaction t = session.beginTransaction();
		String hql = "SELECT DISTINCT lugar FROM Parroquia WHERE lugar LIKE'"
				+ nombre + "%'";
		try {
	...
	@Override
	public Parroquia listarIdParroquia(String nombre) {
		Session session = HibernateUtil.getSf().openSession();
		String hql = "FROM Parroquia WHERE lugar=:nombre";
		Query q = session.createQuery(hql);
		q.setParameter("nombre", nombre);
		return (Parroquia) q.uniqueResult();
	}
