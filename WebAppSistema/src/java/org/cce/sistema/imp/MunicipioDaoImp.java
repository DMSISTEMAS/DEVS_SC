...
public class MunicipioDaoImp implements MunicipioDao{
    ...
    public List<String> listarMunicipio(String nombre) {
        ...
        Session session = HibernateUtil.getSf().openSession();
        Transaction t = session.beginTransaction();
        String hql = "SELECT DISTINCT nombre FROM Municipio WHERE nombre LIKE'" + nombre + "%'";
        try {
	...
	@Override
	public Municipio listarIdMunicipio(String nombre) {
		Session session = HibernateUtil.getSf().openSession();
        String hql = "FROM Municipio WHERE nombre=:nombre";
        Query q = session.createQuery(hql);
        q.setParameter("nombre", nombre);
        return (Municipio) q.uniqueResult();
	}
